<template>
  <div offer-coach :class="{dimOpen:isDim}">
    ...
    <section :class="{'selected':selected}">
          ...
          <div class="profile">
            <div class="info-wrap">
              <div class="thumb-wrap">
                <div class="thumb">
                  <img src="~@/assets/images/mercenary/coach.png" alt="">
                </div>
      ...
      <div class="scout-exp" :class="{on:scoutExp}">
        <p class="tit">스카웃 비용 제안</p>
        <a class="btn exp-btn" @click="scoutExpOpen">
          <span v-if="!scoutExp">스카웃 비용을 작성하세요</span>
          <span v-else>{{scoutExp | comma }}원</span>
        </a>

        <ul>
          <li>용병을 진행하면서 지도 및 피드백도 함께 진행됩니다.</li>
          <li>용병 활동이 완료되기 이전까지는 비용을 사커하우에서 보관하고 있으니 코치님의 불이행을 걱정하시기 않으셔도 됩니다.</li>
        </ul>
      </div>
    ...
    <!--스카웃 비용 제안-->
    <section class="edit-page scout-exp-edit" :class="{open:isScoutExp}">
      <div class="top-btns type2">
        <a class="close" @click="scoutExpClose">닫기</a>
      </div>

      <div class="exp-wrap">
        <p class="tit">스카웃 비용</p>
        <label>
          <input type="number" placeholder="숫자만 입력하세요" v-model="scoutExp">
          <span>원</span>
        </label>
      </div>

      <div class="btns">
        <a class="btn" @click="scoutExpClose">완료</a>
      </div>
    </section>
...
export default {
  ...
  data() {
    return {
      info: [
        { name:'김뮌재', pos:'코치', height: 178, foot: '양발', posKr: '레프트 윙 포워드', addr: '서울시 마포구', exp: '76,500,000' },
      ],
      trigger: false,
      isDim: false,
  ...
  methods: {
    scoutExpOpen() {
      this.isScoutExp = true;
      this.isDim = true;
    },
    scoutExpClose() {
      this.isScoutExp = false;
      this.isDim = false;
    },
    scheduleOpen() {
    ...
    offerBtnActive() {
      if(this.selected === true && this.scoutExp.length >= 0 && this.msgPush === true) {
...
<style scoped>
  *{font-family:var(--global-font-family); color: #101014;}
  ...
  .profile-wrap, .schedule-info, .scout-exp, .msg { position: relative; padding: 30px 20px 20px; box-sizing: border-box; border-bottom: 1px solid #F5F5F5; }
  .profile-wrap .tit, .schedule-info .tit, .scout-exp .tit, .msg .tit { font-size: 16px; line-height: 23px; font-weight: 500; margin-bottom: 20px; }
  ...
  .scout-exp .exp-btn { width: 100%; height: 56px; border: 1px solid #bbb; border-radius: 9px; margin-bottom: 10px; }
  .scout-exp .exp-btn span { font-size: 14px; line-height: 54px; color:#bbb; text-align: center; display: block; }
  .scout-exp.on .exp-btn { border-color: var(--color-main); }
  .scout-exp.on .exp-btn span { color:#101014; }
  .scout-exp ul li { font-size: 12px; line-height: 19px; color:#bbb; margin-bottom: 5px; padding-left: 10px; position: relative; }
  .scout-exp ul li:after { content:''; width: 3px; height: 3px; background: #bbb; border-radius: 50%; position: absolute; left:0; top:8px; }
  .scout-exp ul li:last-child { margin-bottom: 0; }
  ...
  .edit-page.scout-exp-edit { height: 70%; }
  .edit-page.scout-exp-edit .top-btns.type2 { height: 66px; border-bottom:none; }
  .exp-wrap { padding: 20px; box-sizing: border-box; }
  .exp-wrap .tit { font-size: 20px; font-weight: 500; margin-bottom: 30px; }
  .exp-wrap label { width: 100%; height: 60px; border-radius: 5px; border: 1px solid #bbb; display: inline-block; }
  .exp-wrap input { font-size: 20px; font-weight: 500; padding-left: 20px; box-sizing: border-box; background: none; width: calc(100% - 50px); height: 58px; border: none; display: inline-block; vertical-align: top; }
  .exp-wrap label span { font-size: 20px; line-height: 60px; font-weight: 500; text-align: center; color:#606060; display: inline-block; padding-right: 20px; box-sizing: border-box; width: 50px; vertical-align: top; }
  .edit-page.scout-exp-edit .btns { border-top:none; }
  .edit-page.scout-exp-edit .btns .btn { border: 1px solid; border-color: var(--color-main); background: none; color:#101014; }
