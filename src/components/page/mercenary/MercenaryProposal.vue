<template>
  <div proposal>
      ...
      <div class="inner">
        <ul>
          <li class="head">
            <h2>용병 제안</h2>
            <div class="top-txt">
              <span>{{nick}}</span> 회원님으로부터 제안이 도착했어요!
              어떤 제안인지 확인해보세요
            </div>
    ...
    <div class="bottom-btns">
      <p v-if="status === 'wait'">😎 나를 포함한 <span>6명</span>이 제안을 받았어요</p>
      <p v-if="status === 'accept'">👏 스케줄에 용병으로 참여하고 있어요</p>
      <p v-if="status === 'refuse'">😢 용병 제안을 거절했어요</p>
      <p v-if="status === 'end'">👋 이미 끝난 스케줄이거나, 취소된 제안이에요</p>
      <div class="btns">
        <div v-if="status === 'wait'">
          <a class="refuse" @click="active = 'refuse'">거절하기</a>
          <a class="accept" @click="active = 'accept'"><img src="~@/assets/images/mercenary/ico-acc.svg" alt=""> 수락하기</a>
        </div>
...
<style scoped>
  *{font-family:var(--global-font-family); color: #101014;}
  ...
  .bottom-btns a.refuse { background: #101014; }
